...
class AIAgent {
  ...
  async handleWithAI(to, customerPhone, text) {
    ...
    // Build system prompt with detailed product info
    const menuList = SHOP_CONFIG.menu.map(p => 
      `${p.id}. ${p.name} - ${p.price} ${SHOP_CONFIG.currency}${p.description ? ` | ${p.description}` : ''}`
    ).join('\n');
...
2. إذا سأل العميل عن منتج غير موجود، اعتذر بلطف واقترح منتجات مشابهة من القائمة
3. لا تخترع أو تتخيل أي معلومة عن المنتجات - استخدم فقط ما هو مكتوب في القائمة
4. إذا سئلت عن شيء لا تعرفه عن المنتج، قل "للمزيد من التفاصيل يرجى التواصل مع صاحب المتجر"
